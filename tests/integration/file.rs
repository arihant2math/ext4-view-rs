...
async fn test_file_metadata() {
    ...
    let file = fs.open("/small_file").await.unwrap();
...
async fn test_file_read_empty_file() {
    let fs = load_test_disk1().await;
    let mut file = fs.open("/empty_file").await.unwrap();
    let mut buf = [0];
    assert_eq!(file.read_bytes(&mut buf).await.unwrap(), 0);
}
...
async fn test_file_read_exact() {
    ...
    let mut buf = [0; 13];
    assert_eq!(file.read_bytes(&mut buf).await.unwrap(), buf.len());
...
#[tokio::test]
async fn test_file_read_big_buf() {
    let fs = load_test_disk1().await;
    let mut file = fs.open("/small_file").await.unwrap();

    // Read the whole file at once, buffer is larger than the file.
    let mut buf = vec![b'X'; 20];
    assert_eq!(file.read_bytes(&mut buf).await.unwrap(), 13);
...
#[tokio::test]
async fn test_file_read_by_byte() {
    let fs = load_test_disk1().await;
    let mut file = fs.open("/small_file").await.unwrap();
    ...
    let mut all = Vec::new();
    for _ in 0..13 {
        assert_eq!(file.read_bytes(&mut buf).await.unwrap(), 1);
        all.extend(buf);
    }
...
#[tokio::test]
async fn test_file_read_by_twos() {
    let fs = load_test_disk1().await;
    let mut file = fs.open("/small_file").await.unwrap();
    ...
    let mut all = Vec::new();
    for _ in 0..6 {
        assert_eq!(file.read_bytes(&mut buf).await.unwrap(), 2);
        all.extend(buf);
    }
...
async fn test_file_read_holes() {
    ...
    // Check that reading again does not return any bytes.
    assert_eq!(file.read_bytes(&mut all).await.unwrap(), 0);
}

/// Test that each read is limited to at most one block.
#[tokio::test]
async fn test_file_read_limited_to_block() {
    let fs = load_ext2().await;
    // Load a file that is larger than one block.
    let mut file = fs.open("/big_file").await.unwrap();

    let mut buf = vec![0xff; 2048];
    assert_eq!(file.read_bytes(&mut buf).await.unwrap(), 1024);
...
#[tokio::test]
async fn test_file_seek_first_block() {
    let fs = load_test_disk1().await;
    let mut file = fs.open("/small_file").await.unwrap();
    let mut buf = [0; 5];

    file.seek_to(7).await.unwrap();
    assert_eq!(file.read_bytes(&mut buf).await.unwrap(), buf.len());
...
#[tokio::test]
async fn test_file_seek_multiple_blocks() {
    let fs = load_ext2().await;
    // Load a file that is larger than one block.
    let mut file = fs.open("/big_file").await.unwrap();
...
#[tokio::test]
async fn test_file_seek_past_end() {
    let fs = load_test_disk1().await;
    let mut file = fs.open("/small_file").await.unwrap();

    // Seek way past the end of the file.
    file.seek_to(1000).await.unwrap();
...
#[tokio::test]
async fn test_file_open_errors() {
    let fs = load_test_disk1().await;

    assert!(matches!(
        fs.open("not_absolute").await.unwrap_err(),
        Ext4Error::NotAbsolute
    ));
    assert!(matches!(
        fs.open("/empty_dir").await.unwrap_err(),
        Ext4Error::IsADirectory
    ));
}

#[tokio::test]
async fn test_file_debug() {
    let fs = load_test_disk1().await;
    let file = fs.open("/small_file").await.unwrap();
